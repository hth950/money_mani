...
복합 점수는 종목 자체의 **매수 매력도(0~100)** 로 유지하고, 현재
포트폴리오·타이밍·변동성·매크로 이벤트·축간 불일치·데이터 품질은
별도의 **진입 위험도(0~100, 높을수록 위험)** 로 표시합니다.

| 추천 등급 | 기준 | 의미 |
|---|---|---|
| `BUY_READY` | 매력도 ≥65, 위험도 ≤35 | 바로 검토 가능한 매수 추천 |
| `BUY_CONDITIONAL` | 매력도 ≥65, 위험도 36~80 | 위험 사유를 이해한 경우에만 조건부 모의매수 |
| `EARLY_WATCH` | 매력도 55~64, 위험도 ≤60 | 매수 전 관심 후보 |
| `WATCH` | 매력도 40~54 또는 위험도 >80 | 아직 진입하지 않고 관망 |
| `AVOID` | 매력도 <40 | 제외 후보 |
| `UNAVAILABLE` | 가격·데이터 무결성 실패 | 판단 및 매수 불가 |

섹터 집중, 기존 보유, 일일 손실 경고는 추천을 숨기는 하드 차단이
아니라 설명 가능한 위험 점수로 반영합니다. 첫 섹터 진입은 20%,
같은 섹터 두 번째 진입은 40%로 가정합니다. 가격 또는 스냅샷
무결성 실패만 하드 차단합니다.

### 관망 (WATCH)
- `EARLY_WATCH`는 65점까지 부족한 점수와 위험 개선 조건을 표시합니다.
- `WATCH`와 `AVOID`는 모의매수할 수 없고 조건 변화를 기다립니다.
...
| `/signals` | **매매 대시보드** | 매수 추천 / 조건부 / 관심 / 관망·제외 / 매도 추천과 2축 점수 |
...
- 조건부 후보는 위험 사유를 펼쳐 확인하고 `위 위험을 이해했습니다`에 체크해야 미리보기와 체결이 가능합니다
- 위험 스냅샷에는 현재 전략·모의 포트폴리오 상태가 포함됩니다. 다른 체결로 보유 상태가 바뀌면 이전 확인은 409로 무효화되어 최신 위험을 다시 확인해야 합니다
