...
| **Phase 1** | DB status 제약 확장 + get_validated() 수정 + UI 배지 | ⬜ 대기 |
| **Phase 2** | rejected_v2 → archived 소프트 딜리트 (파일 유지) | ⬜ 대기 |
| **Phase 3A~D** | 신규 per-ticker 전략 추가 (ATR추세, 듀얼모멘텀, 섹터로테이션, 수급팔로잉) | ⬜ 대기 |
| **Phase 3E~F** | 팩터 전략 (저변동성, F-Score) — PortfolioRanker 신규 구현 | ⬜ 대기 |
| **Phase 4** | KIS REST API 데이터 전환 (pykrx 대체, 수급 Naver 대체) | ⬜ 대기 |
| **Phase 5** | 검증 + 테스트 | ⬜ 대기 |
| **Phase 6** | main 머지 + 서버 배포 (Phase마다 즉시) | ⬜ 대기 |
...
| strategies.status CHECK에 validated_v2 없음 | ⬜ Phase 1 예정 | `web/db/schema.sql` |
| get_validated()이 validated_v2 미인식 | ⬜ Phase 1 예정 | `strategy/registry.py` |
