...
| 파일 | 역할 |
|---|---|
| `registry.py` | YAML 파일 로드/저장/목록. `get_validated()` → validated + validated_v2 모두 반환 (수정 완료) |
...
**DB 상태**: `strategies.status` CHECK 제약 확장 완료 (validated_v2/rejected_v2/archived/retired 포함)
...
| **Phase 2** | rejected_v2/rejected → archived 소프트 딜리트 (126개) | ✅ 완료 |
| **Phase 3A** | ATR 추세추종 신고가 전략 + atr_stop/highest 인디케이터 (KR+US) | ✅ validated_v2 |
| **Phase 3B** | 듀얼 모멘텀 KR/US 전략 (ROC_252 > 0) | ✅ validated_v2 |
| **Phase 3C** | 섹터 로테이션 KR/US 전략 (ROC_252 > 5% + SMA_20) | ✅ validated_v2 |
| **Phase 3D** | 외국인/기관 수급 팔로잉 (OBV+MFI 프록시, KR only) | ⚠️ draft (WR 18% 미달) |
| **Phase 3E~F** | 팩터 전략 (저변동성, F-Score) — factor_ranker.py 구현 완료 | ⬜ 백테스트 미완 |
| **Phase 4** | KIS REST API 데이터 클라이언트 (15 req/sec, OHLCV+수급) | ✅ 완료 |
| **Phase 5** | 신규 전략 백테스트 + 서버 검증 | ✅ 완료 |
| **Phase 6** | 서버 배포 (Phase마다 즉시 rsync) | ✅ 완료 |

### 현재 전략 현황 (2026-03-16 기준)
- **validated_v2**: 26개 (21개 기존 + 5개 신규: ATR추세추종/듀얼모멘텀KR·US/섹터로테이션KR·US)
- **archived**: 126개 (rejected_v2 80개 + rejected 46개)
- **draft**: 5개 (외국인기관수급팔로잉, 저변동성팩터KR·US, 피오트로스키KR·US)

### 버그 수정 이력 (이번 세션)
- `backtester/signals.py`: `highest` 지표에 `.shift(1)` 추가 (Close > 당일포함 252d high → 항상 False)
- `backtester/signals.py`: `atr_stop` 지표 추가 (ATR 추세추종 전략용)
- `web/db/migrate.py`: strategies 테이블 재생성 후 backtest_results/signals FK 자동 복구
  (SQLite ALTER TABLE RENAME이 FK 참조를 strategies_old로 자동 변경 → DROP 후 깨지는 버그)
- DB 동기화: YAML status → DB 동기화 (101개 validated → validated_v2/archived 정정)
